...
# Form fields
ttk.Label(tab, text="Register New Customer", font=("Arial", 12, "bold")).grid(row=0, column=0,
...
# Form fields
ttk.Label(tab, text="Process Money Transfer", font=("Arial", 12, "bold")).grid(row=0, column=0,
columnspan=2, pady=10)
...
self.summary_tree = ttk.Treeview(tab, columns=("ID", "Name", "Account Type", "DOB", "Area", "Balance"),
show="headings")
# Define headings
...
# Form fields
ttk.Label(tab, text="View Transaction History", font=("Arial", 12, "bold")).grid(row=0, column=0,
columnspan=2, pady=10)
...
self.history_tree = ttk.Treeview(tab, columns=("Date", "Type", "Amount", "Balance", "Recipient", "Txn ID",
"UTR"), show="headings")
# Define headings
...
writer.writerow(["Customer ID", "Name", "Account Type", "DOB", "Address", "Area", "Date", "Type",
"Amount", "Balance", "Recipient ID", "Transaction ID", "UTR"])
...
expected_header = ["Customer ID", "Name", "Account Type", "DOB", "Address", "Area", "Date",
"Type", "Amount", "Balance", "Recipient ID", "Transaction ID", "UTR"]
...
self.transfer_status.config(text=f"Invalid transaction type! Choose from {TRANSACTION_TYPES}",
foreground="red")
return
if amount <= 0:
...
self.transfer_status.config(text=f"Amount exceeds limit of ₹{TRANSFER_LIMIT:.2f}!",
foreground="red")
return
...
details += f"Recipient: {customers[recipient_id]['name']} (ID: {recipient_id},
{customers[recipient_id]['account_type']})\n"
...
text=f"Current Balance for {customer['name']} (ID: {cust_id}, {customer['account_type']}):
₹{customer['balance']:.2f}",
foreground="blue"
)
